...
import { ActionSearchIcon, StyleDeleteIcon } from "@parte-ds/icons";
import { Box, Button, IconButton, TextInput } from "@parte-ds/ui";
import { GridApi, GridReadyEvent } from "ag-grid-community";
import { Fragment, useEffect, useRef, useState } from "react";
import StoreScheduleTable from "../Table/StoreSchedule";

export default function ScheduleContent() {
  const gridApi = useRef<GridApi<StoreSchedule>>();

  const [data, setData] = useState<StoreSchedule[]>();
  ...
  return (
    <Fragment>
      <Layout>
        <Layout.FullHeight autoPadding display="flex" flexDirection="row" paddingTop={16} paddingBottom={24}>
          <Layout.Table display="flex" flexDirection="column" gap={16}>
            <Box display="flex" flexDirection="row" justifyContent="space-between">
              <TextInput width={240} placeholder="일정 타이틀을 검색하세요." leadingIcon={<ActionSearchIcon size={12} color="N600" />} />
              <Box columnGap={8} display="flex">
                <IconButton Icon={<StyleDeleteIcon />} variant="outline-secondary" />
                <Button>매장 일정 생성</Button>
              </Box>
            </Box>
            <StoreScheduleTable data={data} isLoading={isLoading} onGridReady={onGridReady} />
          </Layout.Table>
        </Layout.FullHeight>
      </Layout>
    </Fragment>
